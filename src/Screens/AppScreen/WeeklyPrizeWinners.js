...
const WeeklyPrizeWinners = ({ navigation }) => {
    ...
    const GetPrizeWinnersList = async () => {
        try {
            ...
            setLoading(false);
            console.log('json WeeklyPrizeWinners--->', json);
            if (json.status === "success") {
                setPrizeWinners(json.weekly_winners)
                setPrizeWinners(json.weekly_winners)
                setOffset(offset + 1);
                setDataSource([...dataSource, ...json.weekly_winners]);
            } else {
                alert(json.message)
            }
        } catch (error) {
...
export default WeeklyPrizeWinners
